...
- リスト側の並び替え・距離表示：BEはPostGISで絞り込んだ順（距離順は保証しない）で返却するため、リスト表示時にFE側でカテゴリ内を中心点からの距離順（Haversine公式、`frontend/src/lib/geo.ts`の`distanceMeters`を再利用）にソートし、各項目に距離を併記する。
...
- リストのカテゴリ内は中心点からの距離が近い順に表示し、各スポットに距離（km、小数点第1位まで）を併記する
---
